...
function liveCard() {
  return `<article class="panel"><div class="panel-head"><h2>In voice right now</h2><span class="tag"><span class="status-dot"></span> Live</span></div>${dashboard.live.length?dashboard.live.map(room=>`<div class="room"><span class="room-icon">◉</span><div><strong>${escape(room.channel)}</strong><p>${room.people.map(escape).join(', ')}</p></div>${tag(room.solo?'solo':room.people.length+' together')}</div>`).join(''):empty('The rooms are quiet right now. When someone joins a voice channel, you’ll see them here live.')}</article>`;
}
...
function eventTable(events){ return events.length ? `<div class="table-wrap"><table><thead><tr><th>POLL / POST</th><th>STATUS</th><th>WHEN · BANGKOK</th><th></th></tr></thead><tbody>${eventRows(events)}</tbody></table></div>` : empty('Nothing here yet. Create your first poll or announcement and give everyone something to look forward to.'); }
function overview() {
  ...
  +(failed?`<div class="alert">${failed} post${failed===1?' needs':'s need'} a quick delivery check. Open Polls & posts to review ${failed===1?'it':'them'}.</div>`:'')
...
function createPage(data={}) {
  ...
  <div class="form-footer"><small>Saving a draft is private — nothing is posted to Discord until you publish.</small><button type="submit" class="button">${editKey?'Save changes':'Save draft'} →</button></div></form></section>
...
function schedulePage(){const s=dashboard.settings;return heading('Set it. Let Teemo handle it.','The everyday routine for your server, in Bangkok time.')
  ...
  <div class="form-footer"><small>New times apply from the next run onward. If today's time has already passed, use the buttons on the right.</small><button class="button" type="submit">Save schedule →</button></div></form></section>
  <aside><section class="panel"><div class="panel-head"><h2>Need it right now?</h2></div><p class="hint">Run today's task once, right away. Teemo won't send a duplicate if it already went out.</p><div class="detail-actions">${button("Post today's poll",'run-poll','','secondary')}${button("Post today's summary",'run-report','','secondary')}</div><p class="hint">Posting the summary closes today's daily poll. Pausing these daily tasks does not pause event polls or voice tracking.</p></section><div class="preview-note"><strong>Always in the background</strong><br>Voice sessions and solo periods continue to be recorded. Attendance is kept out of Discord's daily summary.</div></aside></div>`;}
...
async function details(key){$('#detail-content').innerHTML='<div class="loading">Loading details…</div>';$('#detail-dialog').showModal();const {event:e,votes}=await api('events/'+key);const counts=e.options.map((_,i)=>votes.filter(v=>v.choice===i).length);const max=Math.max(...counts,1);$('#detail-content').innerHTML=`<h2 class="detail-title">${escape(e.title)}</h2><div class="detail-meta">${tag(e.status)}<span class="muted">${escape(e.kind)} · #${escape(channelName(e.channel_id))}</span></div><p class="detail-body">${escape(e.body)}</p>
  ...
  ${e.kind==='poll'?`<hr class="divider"><h3>Answers (${votes.length})</h3><div class="detail-result chart">${e.options.map((o,i)=>`<div class="chart-row"><span>${escape(o)}</span><meter value="${counts[i]}" max="${max}" min="0" aria-label="${escape(o)} votes"></meter><span>${counts[i]}</span></div>`).join('')}</div>${votes.length?`<div class="table-wrap"><table><thead><tr><th>MEMBER</th><th>ANSWER</th><th>REASON</th><th>TIME</th></tr></thead><tbody>${votes.map(v=>`<tr><td>${escape(v.display_name)}</td><td>${escape(v.answer)}</td><td class="wrap">${escape(v.reason||'—')}</td><td>${fmtDate(v.at)}</td></tr>`).join('')}</tbody></table></div>`:empty('Answers will appear here as soon as people start voting.')}`:''}<p class="hint">Created ${fmtDate(e.created_at)}${e.history?.length?' · Last action: '+escape(e.history.at(-1).action)+' at '+fmtDate(e.history.at(-1).at):''}</p>`;}
function dailyDetails(key){const poll=dashboard.polls.find(p=>p.id===key);const responses=dashboard.responses.filter(r=>r['Poll Key']===key);$('#detail-content').innerHTML=`<h2 class="detail-title">Daily game poll · ${escape(poll['Poll Date'])}</h2>${tag(poll.Status)}<p class="hint">Only server admins can see these answers and voice attendance.</p><div class="detail-actions">${button('↓ Export answers (CSV)','export','data-kind="responses"')}</div>${responses.length?`<div class="table-wrap"><table><thead><tr><th>MEMBER</th><th>ANSWER</th><th>PLAY TIME</th><th>REASON</th><th>JOINED VOICE CHAT</th></tr></thead><tbody>${responses.map(r=>`<tr><td>${escape(r['Display Name'])}</td><td>${escape(r.Choice)}</td><td>${escape(r['Play Time']||'—')}</td><td class="wrap">${escape(r.Reason||'—')}</td><td>${r['Joined Voice Chat']==='yes'?'✓ Yes':'—'}</td></tr>`).join('')}</tbody></table></div>`:empty('No answers yet.')}`;$('#detail-dialog').showModal();}
...
document.addEventListener('click',async ev=>{
  ...
  try{
    if(action==='detail')await details(key);
      ...
      const actionMessages={publish:'Published to Discord 🎉',close:'Voting is now closed.',cancel:'Cancelled — nothing was posted.',recover:'Delivery check complete.'};
      const {event}=await api(`events/${key}/${op}`,{});await refresh();await details(key);toast(event.status==='review'?'Delivery needs a check. See the event details.':(actionMessages[op]||'Saved.'),event.status==='review');
...
document.addEventListener('submit',async ev=>{
  ...
  try{
    if(form.id==='event-form'){
      ...
      const result=await api('settings',data);session.settings=result.settings;dashboard.settings=result.settings;toast('Daily schedule saved. Teemo will use the new times from now on.');render();
    }
